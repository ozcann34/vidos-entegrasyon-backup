...
DEFAULT_PERMISSIONS = {
    ...
    'xml_products': True,
    'excel_products': True,
    'manual_product_add': True,
    'bulk_excel_update': True,
    'trendyol': True,
    'pazarama': True,
    'hepsiburada': True,
    'n11': True,
    ...
    'auto_sync': True,
    'xml_updates': True,
    'instagram_panel': True,
    'trendyol_questions': True,
    'trendyol_claims': True,
    'blacklist': True,
    'archive': True,
    'reports': True,
    'settings': True,
...
class User(db.Model, UserMixin):
    ...
    def has_permission(self, permission: str) -> bool:
        """Check if user has a specific permission, enforcing global toggles and marketplace limits."""
        # Admins have all permissions (unless it's a critical global system-wide toggle if needed, 
        # but usually admins are exempt)
        if self.is_admin:
            return True
        
        # 1. Check Global Toggles first (via Setting model)
        from app.models.settings import Setting
        global_enabled = Setting.get(f'GLOBAL_ENABLE_{permission}', 'true') == 'true'
        if not global_enabled:
            return False
            
        # 2. Check marketplace specific permissions
        marketplaces = ['trendyol', 'hepsiburada', 'pazarama', 'n11', 'idefix']
        if permission in marketplaces:
            if not self.subscription or not self.subscription.is_active:
                return False
            
            import json
            allowed = []
            if self.subscription.allowed_marketplaces:
                try:
                    allowed = json.loads(self.subscription.allowed_marketplaces)
                except:
                    allowed = []
            
            return permission in allowed

        # 3. Check individual user permissions
    ...
    def is_super_admin(self) -> bool:
        """Check if user is the super admin with full system access."""
        return self.email == 'bugraerkaradeniz34@gmail.com' and self.is_admin
