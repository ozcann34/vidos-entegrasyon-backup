...
def api_clear_all_cache():
    ...
    try:
        ...
        # 1. Clear pending/paused jobs from memory for the CURRENT USER ONLY
        from app.services.job_queue import clear_all_jobs
        jobs_cleared = clear_all_jobs(user_id=current_user.id)
        
        # 1.5 Clear BatchLog DB table (Permanent fix for stuck jobs)
        try:
            db.session.query(BatchLog).filter_by(user_id=current_user.id).delete()
            db.session.commit()
        ...
        # 2. Clear Trendyol caches
        try:
            from app.services.trendyol_service import clear_all_trendyol_caches
            clear_all_trendyol_caches(user_id=current_user.id)
            cleared_items.append("Trendyol Önbelleği")
        except Exception as e:
            logging.error(f"Failed to clear Trendyol caches: {e}")
            
        # 3. Clear Pazarama caches
        try:
            from app.services.pazarama_service import clear_all_pazarama_caches
            clear_all_pazarama_caches(user_id=current_user.id)
            cleared_items.append("Pazarama Önbelleği")
        except Exception as e:
            logging.error(f"Failed to clear Pazarama caches: {e}")

        # 4. Clear N11 caches
        try:
            from app.services.n11_service import clear_all_n11_caches
            clear_all_n11_caches(user_id=current_user.id)
            cleared_items.append("N11 Önbelleği")
        except Exception as e:
            logging.error(f"Failed to clear N11 caches: {e}")
        
        # 5. Clear temp Excel index from database
        Setting.set('_EXCEL_TEMP_INDEX', '', user_id=current_user.id)
        cleared_items.append("Excel İndeksi")
...
@api_bp.route('/api/xml_sources/<int:source_id>/update_costs', methods=['POST'])
@login_required
def api_xml_sources_update_costs(source_id: int):
    """Trigger background job to update local product costs from XML"""
    try:
        row = SupplierXML.query.filter_by(id=source_id, user_id=current_user.id).first()
        if not row:
            return jsonify({'success': False, 'message': 'Kaynak bulunamadı.'}), 404
        
        from app.services.xml_service import perform_xml_cost_update
        
        job_id = submit_mp_job(
            'xml_cost_update', 
            'xml_service',
            lambda jid: perform_xml_cost_update(jid, source_id),
            params={'source_id': source_id, 'user_id': current_user.id}
        )
        return jsonify({'success': True, 'job_id': job_id, 'message': 'Maliyet güncelleme kuyruğa alındı.'})
    except Exception as e:
        logging.exception('XML cost update trigger error')
        return jsonify({'success': False, 'message': str(e)}), 500
...
def api_job_control():
    ...
    try:
        ...
        job = get_mp_job(job_id, user_id=current_user.id)
        if not job:
            return jsonify({'success': False, 'message': 'Job bulunamadı veya erişim yetkiniz yok'}), 404
...
def api_trendyol_sync_stock():
    try:
        ...
        job_id = submit_mp_job(
            'trendyol_sync_stock',
            'trendyol',
            lambda job_id: perform_trendyol_sync_stock(job_id, xml_source_id, user_id=current_user.id),
            params={'xml_source_id': xml_source_id},
        )
...
def api_trendyol_sync_prices():
    try:
        ...
        job_id = submit_mp_job(
            ...
            'trendyol',
            lambda job_id: perform_trendyol_sync_prices(job_id, xml_source_id, user_id=current_user.id),
            params={'xml_source_id': xml_source_id},
        )
...
def api_trendyol_fetch_brands():
    ...
    try:
        ...
        count_data = fetch_and_cache_brands(user_id=current_user.id)
        if count_data.get('success'):
            return jsonify({'success': True, 'message': count_data.get('message')})
        else:
            return jsonify({'success': False, 'message': count_data.get('message')}), 400
    except Exception as e:
        logging.exception("Fetch brands error")
        return jsonify({'success': False, 'message': str(e)}), 500
...
def api_trendyol_sync_all():
    ...
    try:
        ...
        job_id = submit_mp_job(
            'trendyol_sync_all',
            'trendyol',
            lambda job_id: perform_trendyol_sync_all(job_id, xml_source_id, user_id=current_user.id),
            params={'xml_source_id': xml_source_id},
        )
        return jsonify({'success': True, 'job_id': job_id, 'message': 'Stok ve fiyat eşitleme kuyruğa alındı.'}), 202
    except Exception as e:
...
def api_n11_fetch_categories():
    ...
    try:
        ...
        success = fetch_and_cache_n11_categories(user_id=current_user.id)
        if success:
             from app.services.n11_service import get_n11_cache
             cache = get_n11_cache(current_user.id)
             count = len(cache.get('category_list', []))
             return jsonify({
                'success': True, 
    ...
    q = Product.query.filter_by(user_id=current_user.id, is_archived=False)
    if query:
        q = q.filter(
...
def api_xml_source_products():
    ...
    # Advanced Filters
    stock_status = request.args.get('stock_status', 'all')  # all, in_stock, out_of_stock
    image_status = request.args.get('image_status', 'all')  # all, has_image, no_image
    price_sort = request.args.get('price_sort', 'none')  # none, asc, desc
    ...
    # Get archived barcodes for this user to filter them out
    archived_barcodes = set(b[0] for b in db.session.query(Product.barcode).filter_by(user_id=current_user.id, is_archived=True).all())

    # Filter
    filtered = []
    for rec in all_records:
        if rec.get('barcode') in archived_barcodes:
            continue
            
        # 1. Search Query (Title or Barcode)
        if query:
            if query not in rec.get('title_normalized', '') and query not in str(rec.get('barcode', '')).lower():
                continue
        
        # 2. Stock Status Filter
        stock = int(rec.get('quantity') or 0)
        if stock_status == 'in_stock' and stock <= 0:
            continue
        if stock_status == 'out_of_stock' and stock > 0:
            continue
            
        # 3. Image Status Filter
        images = rec.get('images') or []
        has_images = len(images) > 0
        if image_status == 'has_image' and not has_images:
            continue
        if image_status == 'no_image' and has_images:
            continue
...
@api_bp.route('/api/settings/multipliers', methods=['GET'])
@login_required
def api_get_multipliers():
    """Get all marketplace settings (multipliers, brands) for the current user."""
    ...
    mp_key_map = {
        'trendyol': ('TRENDYOL_BRAND_NAME', 'TRENDYOL_BRAND_ID'),
        'hepsiburada': ('HB_BRAND_NAME', 'HB_BRAND_ID'),
        'pazarama': ('PAZARAMA_BRAND_NAME', 'PAZARAMA_BRAND_ID'),
        'n11': ('N11_DEFAULT_BRAND', 'N11_DEFAULT_BRAND_ID'),
        'idefix': ('IDEFIX_BRAND_NAME', 'IDEFIX_BRAND_ID'),
    }

    for mp in ['trendyol', 'hepsiburada', 'pazarama', 'n11', 'idefix']:
        multipliers[mp] = get_marketplace_multiplier(mp, user_id=current_user.id)
        
        name_key, id_key = mp_key_map.get(mp, (None, None))
        if name_key:
            brands[mp] = {
                'name': Setting.get(name_key, "", user_id=current_user.id),
                'id': Setting.get(id_key, "", user_id=current_user.id)
            }
...
def api_marketplace_products(marketplace: str):
    try:
        ...
        if marketplace.lower() == 'trendyol':
            ...
            if use_db:
                # Merge logic to exclude archived products
                query = db.session.query(MarketplaceProduct, Product.id.label('local_product_id')).filter(
                    MarketplaceProduct.user_id == current_user.id,
                    MarketplaceProduct.marketplace == marketplace
                )
                
                # Join with Product to check is_archived
                # Note: We use outerjoin because some marketplace products might not have a local matching Product entry yet
                # but if they DO have one, we respect the is_archived flag.
                # Actually, if we WANT to hide archived products, we should definitely join.
                query = query.outerjoin(Product, (Product.user_id == MarketplaceProduct.user_id) & (Product.barcode == MarketplaceProduct.barcode))
            ...
            if use_db:
                # Join with Product to exclude archived
                query = db.session.query(MarketplaceProduct, Product.id.label('local_id')).filter(
                    MarketplaceProduct.user_id == current_user.id,
                ...
                total_filtered = total_local # Will be overridden if query filtering applies
        ...
        elif marketplace.lower() == 'hepsiburada':
            try:
                from app.services.hepsiburada_service import get_hepsiburada_client
                client = get_hepsiburada_client(user_id=current_user.id)
                
                resp = client.list_products(page=page-1, limit=per_page, search=q or None)
                
                # Parse HB Listing API response
                listings = resp.get('listings') or []
                total_api = int(resp.get('total') or len(listings))
                
                items = []
                for p in listings:
                    # Extract product info from HB listing format
                    barcode = p.get('hbSku') or p.get('merchantSku') or ''
                    stock_code = p.get('merchantSku') or barcode
                    title = p.get('productName') or ''
                    price = float(p.get('price') or 0)
                    sale_price = float(p.get('salePrice') or price)
                    qty = int(p.get('availableStock') or 0)
                    
                    # Status mapping
                    status_raw = str(p.get('listingStatus') or '').upper()
                    ...
                    if status_raw == 'ACTIVE':
                        status_label = 'Satışta'
                        status_color = 'success'
                    elif status_raw in ('PASSIVE', 'INACTIVE'):
                        status_label = 'Pasif'
                        status_color = 'warning'
                    ...
                    # Images (HB returns thumbnail URL)
                    images = []
                    if p.get('imageUrl'):
                        images.append(p.get('imageUrl'))
                ...
                logging.exception("Hepsiburada listing error")
                return jsonify({'total': 0, 'items': [], 'error': f'Hepsiburada API Hatası: {str(ex)}'}), 500
...
def api_product_update_details(marketplace):
    ...
    try:
        ...
        if marketplace == 'trendyol':
            ...
            from app.services.hepsiburada_service import perform_hepsiburada_product_update
            res = perform_hepsiburada_product_update(barcode, data)
            return jsonify(res)
...
def api_product_bulk_update(marketplace):
    ...
    if marketplace == 'n11':
        ...
        res = bulk_update_hepsiburada_stock_price(items)
        return jsonify(res)
    ...
    try:
        ...
        def refresh_task(job_id):
            ...
            with current_app.app_context():
                return refresh_trendyol_cache(job_id, user_id=u_id)
...
def api_trendyol_send_all():
    try:
        ...
        job_id = submit_mp_job(
            'trendyol_send_all',
            'trendyol',
            lambda job_id: perform_trendyol_send_all(job_id, xml_source_id, auto_match=auto_match, match_by=match_by, user_id=current_user.id),
...
def api_hepsiburada_send_all():
    try:
        ...
        # Capture user_id for background job
        ...
        job_id = submit_mp_job(
            ...
            'hepsiburada',
            lambda job_id: perform_hepsiburada_send_all(job_id, xml_source_id, match_by=match_by, user_id=user_id),
            params={'xml_source_id': xml_source_id, 'match_by': match_by}
        )
        ...
        return jsonify({
            'success': True, 
            'batch_id': job_id,
            'message': 'Tüm ürünler Hepsiburada kuyruğuna alındı.'
        })
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500
...
def api_n11_send_all():
    try:
        ...
        job_id = submit_mp_job(
            'n11_send_all',
            'n11',
            lambda job_id: perform_n11_send_all(job_id, xml_source_id, auto_match=auto_match, match_by=match_by, user_id=current_user.id),
            params={'xml_source_id': xml_source_id, 'match_by': match_by}
        )
...
def api_pazarama_send_all():
    try:
        ...
        job_id = submit_mp_job(
            'pazarama_send_all',
            'pazarama',
            lambda job_id: perform_pazarama_send_products(job_id, all_barcodes, xml_source_id, user_id=current_user.id),
            params={'xml_source_id': xml_source_id}
        )
    ...
    try:
        ...
        if marketplace == 'idefix':
            ...
            job_id = submit_mp_job(
                ...
                'trendyol',
                lambda job_id: perform_trendyol_send_products(job_id, barcodes, xml_source_id, auto_match=True, match_by=match_by, title_prefix=title_prefix, user_id=current_user.id),
            ...
            job_id = submit_mp_job(
                ...
                'pazarama',
                lambda job_id: perform_pazarama_send_products(job_id, barcodes, xml_source_id, title_prefix=title_prefix, user_id=current_user.id),
                params={'barcodes': barcodes, 'xml_source_id': xml_source_id, 'requested_marketplace': marketplace}
            )
        elif marketplace == 'n11':
            ...
            job_id = submit_mp_job(
                'n11_send_selected',
                'n11',
                lambda job_id: perform_n11_send_products(job_id, barcodes, xml_source_id, auto_match=True, match_by=match_by, title_prefix=title_prefix, user_id=current_user.id),
                params={'barcodes': barcodes, 'xml_source_id': xml_source_id, 'requested_marketplace': marketplace, 'match_by': match_by}
            )
        elif marketplace == 'hepsiburada':
            from app.services.hepsiburada_service import perform_hepsiburada_send_products
            job_id = submit_mp_job(
                'hepsiburada_send_selected',
                'hepsiburada',
                lambda job_id: perform_hepsiburada_send_products(job_id, barcodes, xml_source_id, title_prefix=title_prefix, match_by=match_by, user_id=current_user.id),
                params={'barcodes': barcodes, 'xml_source_id': xml_source_id, 'requested_marketplace': marketplace}
            )
...
@api_bp.route('/api/marketplace/categories/search/<marketplace>')
@login_required
def search_marketplace_categories(marketplace):
    ...
    try:
        if marketplace == 'trendyol':
            ...
            # HB category search via API is slow/complex, often we use a list. 
            # For now, let's assume we have a way or fetch first N.
            # HB actually doesn't have a simple "search" endpoint for categories in their API, 
            # usually you walk the tree. For manual entry, we can try to find by name.
            return jsonify([]) # Placeholder for HB cat search
        elif marketplace == 'pazarama':
            client = get_pazarama_client()
            ...
            from app.services.idefix_service import _IDEFIX_CAT_TFIDF, ensure_idefix_tfidf_ready
            ensure_idefix_tfidf_ready()
            cats = _IDEFIX_CAT_TFIDF.get('leaf', [])
            results = [c for c in cats if q in c.get('name', '').lower()]
            return jsonify(results[:50])
            
        return jsonify([])
    except Exception as e:
        logging.error(f"Category search error {marketplace}: {e}")
        return jsonify([])

@api_bp.route('/api/marketplace/attributes/<marketplace>/<category_id>')
@login_required
def get_marketplace_attributes(marketplace, category_id):
    user_id = current_user.id
    try:
        if marketplace == 'trendyol':
            client = get_trendyol_client()
            ...
            if not api_key or not api_secret:
                return jsonify({"error": "Idefix API bilgileri eksik"}), 400
            client = IdefixClient(api_key, api_secret, vendor_id)
            return jsonify(client.get_category_attributes(int(category_id)))
        
        return jsonify({"error": "Marketplace not supported"}), 400
    except Exception as e:
        logging.error(f"Error fetching attributes for {marketplace}: {e}")
...
def api_hepsiburada_search_brand():
    ...
    try:
        ...
        if not items:
            return jsonify({'success': False, 'message': f'"{brand_name}" bulunamadı'})
            
        return jsonify({
            'success': True,
            'brands': items, # List of {id, name}
            'message': f'{len(items)} marka bulundu'
        })
    except Exception as e:
        logging.exception(f"[HB] Brand search error: {e}")
        return jsonify({'success': False, 'message': str(e)}), 500
...
@api_bp.route('/api/hepsiburada/import-status/<tracking_id>', methods=['GET'])
def api_hb_import_status(tracking_id):
    """Hepsiburada katalog içe aktarım durumunu sorgula"""
    try:
        from app.services.hepsiburada_service import get_hepsiburada_client
        ...
        logging.error(f"HB status route error: {e}")
        return jsonify({'success': False, 'message': str(e)}), 500
...
def api_auto_sync_settings():
    ...
    try:
        ...
        for marketplace_key in MARKETPLACES.keys():
            auto_sync = AutoSync.get_or_create(marketplace_key, user_id=user_id)
            data = auto_sync.to_dict()
            
            # Load extended settings
            data['xml_source_id'] = Setting.get(f'AUTO_SYNC_XML_SOURCE_{marketplace_key}', user_id=user_id)
            data['match_by'] = Setting.get(f'AUTO_SYNC_MATCH_BY_{marketplace_key}', user_id=user_id) or 'barcode'
...
def api_auto_sync_toggle():
    ...
    try:
        ...
        auto_sync = AutoSync.get_or_create(marketplace, user_id=user_id)
        ...
        if xml_source_id:
            Setting.set(f'AUTO_SYNC_XML_SOURCE_{marketplace}', str(xml_source_id), user_id=user_id)
        if match_by:
            Setting.set(f'AUTO_SYNC_MATCH_BY_{marketplace}', str(match_by), user_id=user_id)
        ...
        message = ""
        if enabled:
            success = add_sync_job(marketplace, user_id, interval_minutes)
            message = f'{MARKETPLACES[marketplace]} için otomatik senkronizasyon aktif edildi.'
        else:
            success = remove_sync_job(marketplace, user_id)
...
def api_auto_sync_logs():
    ...
    try:
        ...
        logs = get_sync_logs(user_id=current_user.id, marketplace=marketplace, limit=limit)
...
def api_excel_list():
    ...
    try:
        ...
        files = list_saved_excel_files(user_id=current_user.id)
...
def api_excel_load(file_id):
    ...
    try:
        ...
        if load_saved_excel(file_id, user_id=current_user.id):
            metadata = get_excel_metadata(file_id, user_id=current_user.id)
...
def api_excel_delete(file_id):
    ...
    try:
        ...
        if delete_excel_file(file_id, user_id=current_user.id):
...
def api_excel_upload():
    ...
    try:
        ...
        try:
            ...
            file_id, metadata = parse_excel_file(tmp_path, user_id=current_user.id, original_filename=original_filename)
            return jsonify({'success': True, **metadata})
        finally:
...
def api_excel_products():
    ...
    try:
        ...
        result = get_excel_products(file_id, user_id=current_user.id, page=page, per_page=per_page, search=search)
...
def api_excel_generate_codes():
    ...
    try:
        ...
        result = bulk_generate_codes(file_id, user_id=current_user.id, indices=indices, prefix=prefix, 
...
def api_excel_update_codes():
    ...
    try:
        ...
        result = update_product_codes(file_id, user_id=current_user.id, indices=indices, barcode=barcode, stock_code=stock_code)
        return jsonify(result)
...
def api_excel_generate_all_codes():
    ...
    try:
        ...
        result = generate_all_random_codes(file_id, user_id=current_user.id, prefix=prefix, code_type=code_type, title_prefix=title_prefix)
        return jsonify(result)
...
def api_excel_brands():
    ...
    try:
        ...
        from app.services.excel_service import load_saved_excel, get_user_excel_cache
        
        # Ensure file is loaded
        load_saved_excel(file_id, user_id=current_user.id)
        
        user_cache = get_user_excel_cache(user_id=current_user.id)
...
def api_excel_send_to_marketplace(marketplace):
    ...
    try:
        ...
        excel_index = build_excel_index(file_id, user_id=current_user.id, title_prefix=title_prefix)
        if not excel_index:
        ...
        if indices and len(indices) > 0:
            ...
            products = get_products_by_indices(file_id, user_id=current_user.id, indices=indices)
            for p in products:
                bc = p.get('barcode', '')
                if bc:
        ...
        if marketplace == 'trendyol':
            ...
            from app.services.hepsiburada_service import perform_hepsiburada_send_products
            job_id = submit_mp_job(
                'excel_send', 'hepsiburada',
                lambda jid: perform_hepsiburada_send_products(jid, barcodes, excel_source_id, title_prefix=title_prefix, match_by=match_by),
                params={'barcodes': barcodes[:5], 'total': len(barcodes)}
            )
        else:
            return jsonify({'success': False, 'message': f'{marketplace} henüz desteklenmiyor'}), 400
