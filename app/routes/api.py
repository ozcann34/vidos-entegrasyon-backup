...
from app.services.job_queue import submit_mp_job, get_mp_job, append_mp_job_log, get_running_job_for_user, control_mp_job
...
@api_bp.route('/api/mp_jobs/<job_id>/control', methods=['POST'])
@login_required
def api_mp_job_control(job_id: str):
    payload = request.get_json(force=True) or {}
    action = payload.get('action')
    if not action:
        return jsonify({'success': False, 'message': 'Aksiyon belirtilmedi.'}), 400
...
# ============================================================
# N11 Advanced Sync Routes
# ============================================================

@api_bp.route('/api/n11/refresh_marketplace', methods=['POST'])
@login_required
def api_n11_refresh_marketplace():
    """
    Manually Refresh N11 Marketplace Data (Force Sync).
    Pulls live data from N11 and mirrors it to local DB, bypassing safety locks.
    """
    try:
        user_id = current_user.id
        from app.services.job_queue import submit_mp_job
        ...
        job_id = submit_mp_job(
            'n11_refresh_marketplace',
            'n11',
            lambda jid: sync_n11_products(user_id, job_id=jid, force=True),
            params={'user_id': user_id, 'force': True}
        )
        return jsonify({'success': True, 'job_id': job_id, 'message': 'Pazaryeri veri yenileme başlatıldı'})
    except Exception as e:
        logging.exception("N11 refresh marketplace error")
        return jsonify({'success': False, 'message': str(e)}), 500


@api_bp.route('/api/n11/advanced_sync', methods=['POST'])
@login_required
def api_n11_advanced_sync():
    """
    Trigger Advanced 4-Step Sync Workflow.
    """
    try:
        user_id = current_user.id
        payload = request.get_json(force=True) or {}
        ...
        if not xml_source_id:
            ...
            xml_source_id = Setting.get('AUTO_SYNC_XML_SOURCE_n11', user_id=user_id)
            
        if not xml_source_id:
            return jsonify({'success': False, 'message': 'XML kaynağı seçilmelidir.'}), 400
            
        from app.services.job_queue import submit_mp_job
        from app.services.n11_service import advanced_n11_sync_workflow
        
        job_id = submit_mp_job(
            'n11_advanced_sync',
            'n11',
            lambda jid: advanced_n11_sync_workflow(jid, xml_source_id, user_id=user_id, match_by=match_by),
            params={'user_id': user_id, 'xml_source_id': xml_source_id, 'type': 'advanced'}
        )
        return jsonify({'success': True, 'job_id': job_id, 'message': 'Gelişmiş senkronizasyon başlatıldı'})
    except Exception as e:
        logging.exception("N11 advanced sync error")
        return jsonify({'success': False, 'message': str(e)}), 500
