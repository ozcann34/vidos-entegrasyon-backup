...
def get_mp_job(job_id: str, user_id: Optional[int] = None) -> Optional[Dict[str, Any]]:
    with _MP_JOBS_LOCK:
        job = _MP_JOBS.get(job_id)
        if job:
            # Check if job belongs to user if provided
        ...
        try:
            query = BatchLog.query.filter_by(batch_id=job_id)
...
def clear_all_jobs(user_id: Optional[int] = None) -> int:
    """Clear jobs from memory. If user_id provided, only clears their jobs."""
    with _MP_JOBS_LOCK:
        if user_id is None:
            count = len(_MP_JOBS)
            _MP_JOBS.clear()
            logging.info(f"Cleared ALL {count} jobs from memory")
            return count
        else:
            to_remove = [jid for jid, jdata in _MP_JOBS.items() if jdata.get('params', {}).get('_user_id') == user_id]
            for jid in to_remove:
                _MP_JOBS.pop(jid, None)
            logging.info(f"Cleared {len(to_remove)} jobs for user {user_id} from memory")
