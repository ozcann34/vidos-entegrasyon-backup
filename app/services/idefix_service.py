...
# Idefix Category TF-IDF Cache per user
_IDEFIX_CAT_TFIDF_BY_USER: Dict[int, Dict[str, Any]] = {}

def get_user_tfidf_cache(user_id: int) -> Dict[str, Any]:
    """Get or initialize TF-IDF cache for a specific user."""
    if user_id not in _IDEFIX_CAT_TFIDF_BY_USER:
        _IDEFIX_CAT_TFIDF_BY_USER[user_id] = {
            "leaf": [],
            "names": [],
            "vectorizer": None,
            "matrix": None
        }
    return _IDEFIX_CAT_TFIDF_BY_USER[user_id]

def prepare_idefix_tfidf(categories: List[Dict[str, Any]], user_id: int):
    """
    Prepare TF-IDF vectorizer and matrix for Idefix categories for a specific user.
    Only uses LEAF categories (categories without subcategories).
    """
    cache = get_user_tfidf_cache(user_id)
    
    if not SKLEARN_AVAILABLE:
        logger.warning(f"[IDEFIX] sklearn yüklü değil, TF-IDF eşleştirme çalışmayacak (User: {user_id})")
        cache.update({"leaf": [], "names": [], "vectorizer": None, "matrix": None})
        return
    ...
    logger.info(f"[IDEFIX] User {user_id}: {len(categories)} kategoriden {len(leaf_categories)} tanesi yaprak (leaf)")
    ...
    cache.update({"leaf": leaf_categories, "names": names, "vectorizer": vec, "matrix": mat})
    logger.info(f"Idefix TF-IDF matrisi hazır (User: {user_id}): {len(names)} yaprak kategori")

def ensure_idefix_tfidf_ready(user_id: int) -> bool:
    """
    ...
    if raw:
        try:
            ...
            if categories:
                prepare_idefix_tfidf(categories, user_id=user_id)
                return True
        except Exception as e:
            logger.error(f"Idefix kategori ağacı yüklenirken hata (User {user_id}): {e}")
    
    # If cache is empty, automatically fetch from API
    logger.info(f"[IDEFIX] Kategori önbelleği boş (User {user_id}), API'den otomatik çekiliyor...")
    try:
        ...
        if result.get('success'):
            ...
            if raw:
                categories = json.loads(raw)
                prepare_idefix_tfidf(categories, user_id=user_id)
                logger.info(f"[IDEFIX] {len(categories)} kategori otomatik yüklendi (User {user_id})")
                return True
    except Exception as e:
        logger.error(f"[IDEFIX] Kategori otomatik çekme hatası (User {user_id}): {e}")
    
    return False

def match_idefix_category_tfidf(query: str, user_id: int, min_score: float = 0.15) -> Optional[int]:
    """
    Find best matching Idefix category using TF-IDF + cosine similarity for a specific user.
    """
    if not query:
        return None
    ...
    try:
        ...
        if score >= min_score:
            ...
            logger.info(f"[IDEFIX] User {user_id}: Kategori eşleşti (skor:{score:.2f}): {query[:30]}... -> {cat_name}")
            return cat_id
        return None
    except Exception as e:
        logger.error(f"[IDEFIX] TF-IDF eşleştirme hatası (User {user_id}): {e}")
        return None

def resolve_idefix_category(product_title: str, excel_category: str, user_id: int, log_callback=None) -> Optional[int]:
    """
    Resolve Idefix category ID from product info with user isolation.
    """
    if not ensure_idefix_tfidf_ready(user_id):
        if log_callback:
            log_callback(f"Idefix kategori listesi boş (User {user_id})! Ayarlardan 'Kategorileri Çek' yapın.", level='warning')
        return None
    ...
    for query in (excel_category, product_title):
        ...
        cat_id = match_idefix_category_tfidf(query, user_id=user_id, min_score=0.10)
        if cat_id:
            if log_callback:
                # Get category name for logging
                cache = get_user_tfidf_cache(user_id)
                for cat in cache.get('leaf', []):
...
def get_idefix_client(user_id: Optional[int] = None) -> IdefixClient:
    ...
    if not api_key or not vendor_id:
        logging.warning("[IDEFIX] API Key or Vendor ID not found in settings for user %s", actual_user_id)
        return None
    
    return IdefixClient(api_key, api_secret, vendor_id)

def perform_idefix_send_products(job_id: str, barcodes: List[str], xml_source_id: Optional[int] = None, title_prefix: str = None, is_manual: bool = False) -> Dict[str, Any]:
    from app.services.job_queue import append_mp_job_log, get_mp_job, update_mp_job
    ...
    for barcode in barcodes:
        ...
        rec = mp_map.get(str(barcode))
        if not rec:
            skipped_count += 1
            skipped_list.append({'barcode': barcode, 'reason': 'Kaynakhartasında bulunamadı'})
            continue
        ...
        # Resolve category
        final_cat_id = None
        if rec.get('is_manual') and rec.get('categoryId'):
            try:
                final_cat_id = int(rec.get('categoryId'))
                append_mp_job_log(job_id, f"Manuel kategori seçimi kullanılıyor: {final_cat_id}")
            except: pass
            
        if not final_cat_id:
             excel_category = rec.get('category') or rec.get('top_category') or ''
             product_title = rec.get('title') or ''
             final_cat_id = resolve_idefix_category(product_title, excel_category)
    ...
    for item in products_to_send:
        ...
        if rec.get('is_manual') and rec.get('marketplace_attributes_json'):
             try:
                 mp_attrs = json.loads(rec.get('marketplace_attributes_json'))
                 for a in mp_attrs:
                     product_attributes.append({
                         "attributeId": int(a.get('id')),
                         "attributeValueId": None,
                         "customAttributeValue": a.get('value')
                     })
                 append_mp_job_log(job_id, f"Manuel özellikler kullanılıyor ({len(product_attributes)} adet)")
             except: pass

        missing_required = []
        
        # If no manual attributes, fallback to defaults
