...
def activate_subscription(user_id: int, plan: str, payment_id: int = None, cycle: str = 'monthly', marketplaces: list = None) -> Optional[Subscription]:
    ...
    import json
    # Use provided marketplaces or default to the first X marketplaces allowed by the plan
    all_marketplaces = ['trendyol', 'hepsiburada', 'pazarama', 'n11', 'idefix']
    limit = plan_details.get('max_marketplaces', 1)
    
    if marketplaces:
        # Enforce limit
        if limit != -1:
            allowed = marketplaces[:limit]
        else:
            allowed = marketplaces
    else:
        # Default logic
        allowed = all_marketplaces if limit == -1 else all_marketplaces[:limit]
    ...
    if subscription:
        ...
        subscription.max_xml_sources = plan_details.get('max_xml_sources', 3) # Fix: use correct key
        subscription.allowed_marketplaces = allowed_json
        subscription.billing_cycle = cycle
        subscription.price_paid = plan_details['price']
        if payment_id:
        ...
        subscription = Subscription(
            ...
            max_xml_sources=plan_details.get('max_xml_sources', 3), # Fix: use correct key
            allowed_marketplaces=allowed_json,
