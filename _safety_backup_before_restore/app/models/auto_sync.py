...
class AutoSync(db.Model):
    ...
    @staticmethod
    def get_or_create(marketplace: str, user_id: int) -> 'AutoSync':
        """Get or create AutoSync record for marketplace and user"""
        sync = AutoSync.query.filter_by(marketplace=marketplace, user_id=user_id).first()
        if not sync:
