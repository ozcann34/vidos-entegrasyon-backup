...
def sync_marketplace_products(marketplace: str, user_id: Optional[int] = None, job_id: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    logger.info(f"Starting sync for {marketplace} (User ID: {user_id})")
    ...
    try:
        ...
        # If user_id is None, this might be from a scheduled job that needs to be refactored 
        # to handle multiple users. For now, we prioritize the passed user_id.
        
        sync_record = None
        if user_id:
            sync_record = AutoSync.query.filter_by(marketplace=marketplace, user_id=user_id, enabled=True).first()
        else:
            # Fallback for old scheduler logic (if still active) - NOT SAFE for multiple users
        ...
        _save_sync_log(marketplace, result, user_id=user_id, error_message=sync_res.get('message') if not result['success'] else None)
...
def _save_sync_log(marketplace: str, result: Dict[str, Any], user_id: Optional[int] = None, error_message: Optional[str] = None):
...
def get_sync_logs(user_id: int, marketplace: Optional[str] = None, limit: int = 50) -> List[Dict[str, Any]]:
    """Senkronizasyon loglarını getir"""
    try:
        query = SyncLog.query.filter_by(user_id=user_id)
