...
# ---------------------------------------------------
# N11 Category Caching & Auto Match Globals - USER ISOLATED
# ---------------------------------------------------
_N11_USER_CACHES: Dict[int, Dict[str, Any]] = {}

def get_n11_cache(user_id: int) -> Dict[str, Any]:
    """Get or initialize the N11 cache for a specific user."""
    if user_id not in _N11_USER_CACHES:
        _N11_USER_CACHES[user_id] = {
            "category": {
                "by_id": {}, 
                "list": [], 
                "loaded": False,
    ...
    return _N11_USER_CACHES[user_id]

def load_n11_categories_from_db(user_id: int) -> bool:
    try:
        data = Setting.get("N11_CATEGORY_CACHE", "", user_id=user_id)
        if data:
            import json
            j = json.loads(data)
            cache = get_n11_cache(user_id)
            cache["category"]["by_id"] = {int(k): v for k,v in j.get("by_id", {}).items()}
            cache["category"]["list"] = j.get("list", [])
            cache["category"]["loaded"] = True
            cache["category"]["timestamp"] = j.get("timestamp", 0)
            return True
    except Exception as e:
        logging.warning(f"Failed to load N11 categories from DB for user {user_id}: {e}")
    return False

def save_n11_categories_to_db(user_id: int):
    try:
        cache = get_n11_cache(user_id)
        payload = {
            "by_id": cache["category"]["by_id"],
            "list": cache["category"]["list"],
            "timestamp": time.time()
        }
        Setting.set("N11_CATEGORY_CACHE", json.dumps(payload), user_id=user_id)
    except Exception as e:
        logging.error(f"Failed to save N11 categories to DB for user {user_id}: {e}")

def fetch_and_cache_n11_categories(user_id: int, force=False):
    """Fetch all N11 categories and build cache, user isolated."""
    cache = get_n11_cache(user_id)
    if not force and cache["category"]["loaded"]:
        return True
    
    if not force and load_n11_categories_from_db(user_id):
        if not cache["tfidf"]["vectorizer"]:
            _build_n11_tfidf(user_id)
        return True
    ...
    try:
        ...
        cache["category"]["by_id"] = by_id
        cache["category"]["list"] = flat_list
        cache["category"]["loaded"] = True
        
        save_n11_categories_to_db(user_id)
        _build_n11_tfidf(user_id)
        logging.info(f"N11 Categories cached for user {user_id}: {len(flat_list)} leaf categories.")
        return True

    except Exception as e:
        logging.error(f"Error fetching N11 categories for user {user_id}: {e}")
        return False

def _build_n11_tfidf(user_id: int):
    """Build TF-IDF matrix for N11 categories - USER ISOLATED"""
    try:
        cache = get_n11_cache(user_id)
        leafs = cache["category"]["list"]
        if not leafs:
            return
        ...
        cache["tfidf"]["leaf"] = leafs
        cache["tfidf"]["names"] = names
        cache["tfidf"]["vectorizer"] = vec
        cache["tfidf"]["matrix"] = matrix
        
    except Exception as e:
        logging.error(f"N11 TF-IDF build error for user {user_id}: {e}")

def find_matching_n11_category(query: str, user_id: int) -> Optional[Dict[str, Any]]:
    """Find best matching N11 category for a given query string (product name/category) - USER ISOLATED."""
    cache = get_n11_cache(user_id)
    
    # Ensure loaded
    if not cache["category"]["loaded"]:
        fetch_and_cache_n11_categories(user_id)
        
    # Ensure vectorizer built (even if loaded from DB)
    if not cache["tfidf"]["vectorizer"] and cache["category"]["list"]:
        _build_n11_tfidf(user_id)
    ...
    try:
        ...
        if score > 0.4: 
            match = cache["tfidf"]["leaf"][best_idx]
...
# ---------------------------------------------------
# Attribute & Brand Matching Support - USER ISOLATED
# ---------------------------------------------------

def get_n11_category_attributes(category_id: int, user_id: int):
    """Fetch attributes for a category (Cached in user-specific memory)."""
    cache = get_n11_cache(user_id)
    if "attributes" not in cache:
        cache["attributes"] = {}
        
    if category_id in cache["attributes"]:
        return cache["attributes"][category_id]
    ...
    try:
        ...
        cache["attributes"][category_id] = attrs
        return attrs
    except:
        return []
...
def fetch_all_n11_products(user_id: int, job_id: Optional[str] = None) -> List[Dict[str, Any]]:
    from app.services.n11_client import get_n11_client
    client = get_n11_client(user_id=user_id)
    if not client:
        return []
...
def perform_n11_send_products(job_id: str, barcodes: List[str], xml_source_id: Any = None, auto_match: bool = False, send_options: Dict[str, Any] = None, match_by: str = 'barcode', title_prefix: str = None, is_manual: bool = False, user_id: int = None) -> Dict[str, Any]:
    ...
    multiplier = get_marketplace_multiplier('n11', user_id=user_id) or 1.0
    u_id = user_id or (current_user.id if current_user and current_user.is_authenticated else None)
    
    # Load global settings for N11 - USER ISOLATED
    shipment_template = Setting.get("N11_DEFAULT_SHIPMENT_TEMPLATE", "Standart", user_id=u_id)
    
    default_brand_setting = Setting.query.filter_by(key="N11_DEFAULT_BRAND", user_id=u_id).first()
    default_brand = default_brand_setting.value if default_brand_setting and default_brand_setting.value else None
    
    default_brand_id_setting = Setting.query.filter_by(key="N11_DEFAULT_BRAND_ID", user_id=u_id).first()
    ...
    for barcode in barcodes:
        ...
        if product.get('is_manual') and product.get('categoryId'):
            try:
                cat_id = int(product.get('categoryId'))
    ...
    for item in matched_products:
        ...
        payload_item = {
            ...
            "productMainId": target_code, # Mandatory for grouping variants, unique for single
        ...
        try:
            ...
            if target_user_id:
                ...
                # Commit every 25 to be safe
                if len(items_to_send) % 25 == 0:
                    db.session.commit()
