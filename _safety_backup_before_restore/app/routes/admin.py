...
def super_admin_required(f):
    """Decorator to require super admin access (bugraerkaradeniz34@gmail.com only)."""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if not current_user.is_authenticated:
            flash('Bu sayfaya erişmek için giriş yapmalısınız.', 'warning')
        ...
        if not current_user.is_super_admin:
            flash('Bu işlem için Süper Admin yetkisi gereklidir.', 'danger')
...
def toggle_admin(user_id):
    ...
    status_msg = "Destek ekibine eklendi" if user.is_admin else "Destek ekibinden çıkarıldı"
    flash(f'{user.full_name or user.email} kullanıcısı {status_msg}.', 'success')
    return redirect(url_for('admin.users'))


@admin_bp.route('/users/join_support', methods=['GET', 'POST'])
@super_admin_required
def join_support_team():
    """Add a user to support team (Super Admin only)."""
    if request.method == 'POST':
        ...
        try:
            ...
            AdminLog.log_action(
                admin_id=current_user.id,
                action='join_support_team',
                target_user_id=user.id,
                details=f'Added to support team: {email}',
                ip_address=request.remote_addr
            )
            
            db.session.commit()
            flash('Kullanıcı destek ekibine başarıyla eklendi.', 'success')
            return redirect(url_for('admin.users'))
...
def update_subscription_route(user_id):
    ...
    if update_subscription(user_id, plan, end_date, max_products=max_products, max_xml_sources=max_xml_sources):
        ...
        AdminLog.log_action(
            ...
            target_user_id=user_id,
            details=f'Plan: {plan}, Cycle: {billing_cycle}, Price: {price_paid}, End: {end_date_str or "Unlimited"}',
            ip_address=request.remote_addr
        )
...
def subscriptions():
    ...
    subscriptions = query.join(User).order_by(Subscription.created_at.desc()).paginate(page=page, per_page=per_page, error_out=False)
...
def user_permissions(user_id):
    ...
    # Available permission pages with Turkish labels grouped by category
    available_permissions = {
        ...
        'Ürün İşlemleri': {
            ...
            'manual_product_add': 'Manuel Ürün Ekle',
            'bulk_excel_update': 'Toplu Excel Güncelle',
            'xml_updates': 'XML Güncellemeleri',
            'archive': 'Ürün Arşivi',
        },
        'Pazaryerleri': {
            'trendyol': 'Trendyol',
        ...
        'Pazaryeri Araçları': {
            ...
            'trendyol_questions': 'Müşteri Soruları (Trendyol)',
            'trendyol_claims': 'İade Talepleri (Trendyol)',
            'blacklist': 'Yasaklı Liste',
            'order_actions': 'Sipariş Paketi/Etiket İşlemleri',
        },
        'Ekstralar': {
            'instagram_panel': 'Instagram Paneli',
    ...
    flat_permissions = {}
    for cat, perms in available_permissions.items():
        flat_permissions.update(perms)

    if request.method == 'POST':
        # Get submitted permissions
        new_permissions = {}
        for perm_key in flat_permissions.keys():
...
@super_admin_required
def system_settings():
    """Manage global system settings and feature toggles."""
    from app.models.settings import Setting

    # Define features that can be toggled globally
    features = {
        'Ana Sayfalar': {
            'dashboard': 'Kontrol Paneli',
            'orders': 'Siparişler',
    ...
    if request.method == 'POST':
        # Flattened features for saving
        flat_features = {}
        for category in features.values():
            flat_features.update(category)

        for key in flat_features.keys():
            # Save as 'true' or 'false'
            value = 'true' if request.form.get(f'global_{key}') == 'on' else 'false'
            Setting.set(f'GLOBAL_ENABLE_{key}', value)

        AdminLog.log_action(
            admin_id=current_user.id,
            action='system_settings_update',
            details='Global feature toggles updated',
            ip_address=request.remote_addr
        )
    ...
    for category in features.values():
        for key in category.keys():
            current_values[key] = Setting.get(f'GLOBAL_ENABLE_{key}', 'true') == 'true'
...
def activity_logs():
    ...
    if user_id_filter:
        query = query.filter(UserActivityLog.user_id == user_id_filter)
        
    if marketplace_filter:
        query = query.filter(UserActivityLog.marketplace == marketplace_filter)
    
    logs = query.order_by(UserActivityLog.created_at.desc()).paginate(page=page, per_page=per_page, error_out=False)
    
    # Get distinct actions for filter dropdown
    actions = db.session.query(UserActivityLog.action).distinct().all()
    actions = [a[0] for a in actions]
    
    # Get distinct marketplaces for filter dropdown
    marketplaces = db.session.query(UserActivityLog.marketplace).distinct().all()
    ...
    return render_template('admin/activity_logs.html', 
                           ...
                           current_marketplace=marketplace_filter)
